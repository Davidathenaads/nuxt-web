...
#header
  header
    div(name="mobilemenu")
      div(class="fixed left-[-100%] top-0 z-10 m-0 h-[100vh]  overflow-scroll bg-yoga-mobilecolor p-0  transition-all delay-0 duration-300" :class="{open: menuOpen}")
        div(class="absolute right-5 top-9")
          el-icon(:size="50" color="#fff" class="z-20" @click="Mobilebtnclick")
            Close

        ul(class="text-white")
          li(class="m-5 py-2 text-sm") 品牌介紹 Introduction
    ...
    div(class="my-5 flex items-center justify-around text-white xl:flex")
      div(class=" relative h-4 w-5 justify-start sm:hidden" @click="Mobilebtnclick")
        span(class="absolute left-0 block h-[1px]  rounded-[9px] bg-white opacity-100")
        span(class="absolute left-0 top-[6px] block h-[1px]  rounded-[9px] bg-white opacity-100")
        span(class="absolute left-0 top-3 block h-[1px]  rounded-[9px] bg-white opacity-100")
      a(href="" class=" flex select-none items-center xl:flex")
            ...
            div(class="dropdown-menu absolute z-20 ml-7 hidden pt-12 text-sm")
              ul(class=" -mt-1  mb-0 min-w-[220px]  pl-0")
                li(class=" hover:opacity-80")
                  a(href="#" class="inline-block  min-w-[220px] max-w-[360px] border-b border-solid border-[#f0c4c3]  bg-[#F09491] px-4 pb-3 pt-[10px]") FLY HIGH YOGA
                li(class=" hover:opacity-80")
